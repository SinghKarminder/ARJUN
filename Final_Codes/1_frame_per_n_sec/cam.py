...
@abstract: frames individually after a fixed inteval of seconds
...
# interval after which images to capture
IMAGE_CAP_INTERVAL = 10
#---------------------
...
class MotionRecorder(object):
    ...
    def start_storing_img(self, img):
        ...
        if self.last_minute == None:
            ...
            self.img_count = 0
            self.last_second = self.last_second - 60

      
        # if a change in second then append mean
        # save mean every second for which interval decided
        if self.last_second == None:
            self.last_second = now.second
        elif self.last_second + IMAGE_CAP_INTERVAL <= now.second:
            # if sec changes, save last second count data            
